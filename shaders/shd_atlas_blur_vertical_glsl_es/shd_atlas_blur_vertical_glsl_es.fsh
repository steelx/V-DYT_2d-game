...
vec4 blur_vertical(vec2 uv, vec2 texel)
{
	vec4 s0 = texture2D(gm_BaseTexture, uv) * 0.227027;
	vec4 s1 = texture2D(gm_BaseTexture, vec2(uv.x, uv.y + texel.y)) * 0.1945946;
    vec4 s2 = texture2D(gm_BaseTexture, vec2(uv.x, uv.y - texel.y)) * 0.1945946;
	vec4 s3 = texture2D(gm_BaseTexture, vec2(uv.x, uv.y + texel.y * 2.0)) * 0.1216216;
	vec4 s4 = texture2D(gm_BaseTexture, vec2(uv.x, uv.y - texel.y * 2.0)) * 0.1216216;	
    vec4 s5 = texture2D(gm_BaseTexture, vec2(uv.x, uv.y + texel.y * 3.0)) * 0.054054;
    vec4 s6 = texture2D(gm_BaseTexture, vec2(uv.x, uv.y - texel.y * 3.0)) * 0.054054;	
    vec4 s7 = texture2D(gm_BaseTexture, vec2(uv.x, uv.y + texel.y * 4.0)) * 0.016216;
    vec4 s8 = texture2D(gm_BaseTexture, vec2(uv.x, uv.y - texel.y * 4.0)) * 0.016216;	
...
vec4 shadow_blur(vec2 uv, vec2 size, vec2 atlas, vec2 map_size, sampler2D sample_depth)
{
	...
	vec2 uv0 = clamp(vec2(uv.x, uv.y + size.y), uv_min, uv_max);
	vec2 uv1 = clamp(vec2(uv.x, uv.y - size.y), uv_min, uv_max);
	vec2 uv2 = clamp(vec2(uv.x, uv.y + size.y * 2.0), uv_min, uv_max);
	vec2 uv3 = clamp(vec2(uv.x, uv.y - size.y * 2.0), uv_min, uv_max);
	vec2 uv4 = clamp(vec2(uv.x, uv.y + size.y * 3.0), uv_min, uv_max);
	vec2 uv5 = clamp(vec2(uv.x, uv.y - size.y * 3.0), uv_min, uv_max);
	vec2 uv6 = clamp(vec2(uv.x, uv.y + size.y * 4.0), uv_min, uv_max);
	vec2 uv7 = clamp(vec2(uv.x, uv.y - size.y * 4.0), uv_min, uv_max);
